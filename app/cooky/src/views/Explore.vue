<template>
    <div class="explore">
        ...
        <v-card>
            <!-- Tabs to switch from All recommendations to pantry only recommendation -->
            <v-tabs v-model="tab" background-color="transparent" grow>
                <v-tab key="Tab1" @click="process_all_reco(0)">
                    All
                </v-tab>
    
                <v-tab key="Tab2" @click="process_all_reco(1)">
                    Pantry
                </v-tab>
                    ...
                    <v-container fluid>
                        ...
                        <v-row dense>
                            <v-col v-for="card in cards" :key="card.id" :cols="card.flex">
                                <v-card @click="itemClick(card.id)">
                                    <v-img :src="card.src" class="white--text align-end" gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)" height="200px" lazy-src>
        ...
        <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
            ...
            <v-card>
                ...
                <v-img :src="detail.src" class="white--text" gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)" lazy-src>
                    ...
                    <v-btn class="ma-2" outlined color="white" @click="cookDish(detail.id)" v-if="tab">Cook This Dish</v-btn>
    
    
                    <v-rating hover background-color="grey" color="white" empty-icon="mdi-star-outline" full-icon="mdi-star" half-icon="mdi-star-half-full" length="5" size="30" :value=detail.rating></v-rating>
                ...
                <v-list three-line>
                    ...
                    <v-list-item>
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle v-for="ingredient in detail.ingredients.replace('[','').replace(']','').replace(/&quot;/g, '').split(',')">
                                {{ingredient}}
                    ...
                    <v-list-item>
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle v-for="direction in detail.directions.replace('[','').replace(']','').replace(/&quot;/g, '').split(',')">
                                {{direction}}
                            </v-list-item-subtitle>
                        </v-list-item-content>
                    </v-list-item>
        ...
        <!-- Snackbar on Success -->
        <v-snackbar v-model="on_success.snackbar" :timeout="on_success.timeout">
            {{ on_success.text }}
    
    
            <v-btn color="blue" text @click="on_success.snackbar = false">
                Close
            </v-btn>
...
export default {
    ...
    data() {
        return {
            ...
            all_reco: {},
            pantry_reco: {},
            cards: [], // Cards are loaded via server request

            // Sample content for detailed view
            detail: {
                id: 0,
                title: 'Dish Title',
                src: 'https://cdn.vuetifyjs.com/images/cards/train.jpg',
                directions: 'Description for this dish',
                ingredients: 'Recipe for this dish',
                rating: 5
            },
    ...
    methods: {
        onload() {
            // check if session exists and load data
        ...
        loadRecommedations(session_id) {
            // Fetch recommendations for current session from 
        ...
        process_all_reco(reco_type) {
            ...
            // images are hard-coded as to not expose any API keys. 
            // We recommend using the Bing Image Search Api for production
            // with the recipe title as an input. Take any of the returned
            // URLs as a showcase image.
            ...
            if (Object.values(recommendation).length != 0) {
                ...
                for (let index = 0; index < Object.values(recommendation.n_recipe_id).length; index++) {
                    let card = {
                        id: recommendation.n_recipe_id[index],
        ...
        async itemClick(id) {
            // Fetch rating for detailed view
            let session_id = VueCookies.get("session")
        ...
        cookDish(n_recipe_id) {
            // Cook dish (reduce items in pantry)
            // Only available for Recommendation by pantry
            console.log("cooking")
