...
<template>
    <div>
        ...
        <v-form v-model="valid" v-if="!isHidden">
            ...
            <v-container>
                <v-row>
                    ...
                    <v-col cols="12" md="4">
                        <v-text-field v-model="formData.password" label="password" required :append-icon="show ? 'mdi-eye' : 'mdi-eye-off'" :type="show ? 'text' : 'password'" @click:append="show = !show"></v-text-field>
                    </v-col>
                </v-row>
            </v-container>
        ...
        <!-- Account View, when session is established -->
        <v-skeleton-loader type="card-avatar, article, actions" v-if="isHidden">
        </v-skeleton-loader>
...
export default {
    ...
    methods: {
        ...
        loginPost() {
            // send log-in data to server to establish session
                ...
                axios.post('http://localhost:5000/login', this.formData)
                    .then(function(response) {
                        if (response.status == 200) {
                            ...
                            // on success, redirects to explore view
